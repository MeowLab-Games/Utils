...
namespace MeowLab.Utils
{
    public static class RectExtensions
    {
        ...
        public static Rect Left(this Rect source, float percent) {
            return new Rect(source.x, source.y, source.width * percent, source.height);
        }


        public static Rect Right(this Rect source, float percent) {
            var w = source.width * percent;
            return new Rect(source.x + (source.width - w), source.y, w, source.height);
        }


        public static (Rect, Rect) SplitRightFixed(this Rect source, float w2) {
            // var w = source.width ;
            var w1 = source.width - w2;
            var r1 = new Rect(source.x, source.y, w1, source.height);
            var r2 = new Rect(source.x + (source.width - w2), source.y, w2, source.height);
            return (r1, r2);
        }
